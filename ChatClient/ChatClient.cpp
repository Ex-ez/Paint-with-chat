...
int OnServerMessage(CurrentClientNetworkData* ap_data, void* ap_this, int a_client_index)
{
	if (ap_data->m_net_msg_id == 1) { // 채팅 데이터가 전달됨
	 ...
	 // 전달된 데이터를 선 그리기 정보 구조체인 DrawLineData로 형 변환해서 사용!
		DrawLineData* p_line_data = (DrawLineData*)ap_data->mp_net_body_data;
		// 시작 점과 끝점 그리고 선 색상, 선 굵기를 반영해서 선을 그린다.
		...
		Clear(0, RGB(155, 187, 212)); // 윈도우의 배경을 RGB(82, 97, 124) 색으로 채운다!
		ShowDisplay(); // 정보를 윈도우에 출력한다.
	}
	return 1;
}
...
void OnConnectBtn(AppData* ap_data)
{
	...
	if (!IsConnect(ap_data->p_socket)) {  // 서버와의 접속 상태를 체크한다.
		// "192.168.0.118" 주소에서 25001번 포트로 서비스 중인 서버에 접속을 시도한다.
...
void OnClearImageBtn(AppData* ap_data)
{
	// 클라이언트 소켓 객체가 만들어져 있고 서버와 접속상태인지 확인한다.
	if (ap_data->p_socket && IsConnect(ap_data->p_socket)) {
		// 3번 네트워크 메시지는 '그림 전체 지우기' 기능으로 사용한다.
...
void OnChangeColorBtn(AppData* ap_data, INT32 a_notify_code, void* ap_ctrl)
{
	if (a_notify_code == LBN_SELCHANGE) {
		// 사용자가 선택한 선 색상을 프로그램 내부 데이터에 저장합니다.
...
{
	if (a_notify_code == LBN_SELCHANGE) {
		// 사용자가 선택한 선 굵기를 프로그램 내부 데이터에 저장합니다.
...
void SendLineData(AppData* ap_data, POINT a_pos)
{
	...
	if (ap_data->p_socket && IsConnect(ap_data->p_socket)) {
		...
		// 선 그리기 정보를 서버로 전송한다. 이 기능의 메시지 아이디는 2로 지정한다.
...
void DrawThickItem(int index, char* ap_str, int a_str_len, void* ap_data, int a_is_selected, RECT* ap_rect)
{
	...
	// 선 굵기를 출력하기 위해 항목의 배경 크기게 맞게 사각형을 그린다.
...
{
	COLORREF color_table[21] = {   // 표준 20가지 색상 목록
...
int main()
{
	...
	//CreateListBox(10, 100, 500, 100, 1014); 
	CreateListBox(10, 400, 500, 100, 1000);  // 채팅 목록! 10, 36, 500, 100, 1000
	void* p = CreateListBox(520, 400, 56, 114, 1001, DrawColorItem, LBS_MULTICOLUMN); // 선 색상 목록!
	AddColorToList(p); // 20개의 색상을 추가한다.
	// 선 굵기를 선택하는 리스트 박스를 만들고 1 ~ 7까지 굵기를 추가한다.
	p = CreateListBox(586, 400, 56, 114, 1002, DrawThickItem); // 선 굵기 목록! 586, 36, 56, 114, 1002
	for (int i = 0; i < 7; i++) ListBox_SetItemDataEx(p, i, "", i + 1, 0); // 7가지 선 굵기를 추가한다.
	ListBox_SetCurSel(p, 1);  // 1번 위치에 있는 항목을 선택한다.
	...
	TextOut(15, 380, RGB(82, 97, 124), "사용자 채팅글 목록");  // 채팅 목록 리스트 박스의 제목을 출력 ,16
	TextOut(525, 380, RGB(82, 97, 124), "선 색상");  // 선 색상 선택 리스트 박스의 제목을 출력
	TextOut(591, 380, RGB(82, 97, 124), "선 굵기");  // 선 굵기 선택 리스트 박스의 제목을 출력 200,255,200\
	
// 장기판을 그린다.
	/*SelectPenObject(RGB(0, 0, 0));
	SelectBrushObject(RGB(244, 176, 93));
	Rectangle(650, 00, 750, 700);// X시작, Y시작, X끝, Y높이
	*/
